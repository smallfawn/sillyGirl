...
RUN mkdir -p /usr/local/sillyGirl \
    ...
    && curl -sSL --connect-timeout 10 -f "https://github.com/smallfawn/sillyGirl$DOWNLOAD_URL" -o /usr/local/sillyGirl/sillyGirl) \
...
# docker build -t smallfawn/sillygirl .
# docker run -d --restart always --name sillygirl smallfawn/sillygirl
