...
2. **平台无关**：通过 Adapter 抽象屏蔽底层平台差异，同一插件可同时服务于 QQ、Telegram、Web 等多个平台
...
│                              Client Layer                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐ │
│  │ QQ/TG/微信Bot│  │ Web Browser │  │  gRPC Client│  │  Pagermaid Bot  │ │
│  │ (WS/API)    │  │  (Admin UI) │  │  (Python/JS)│  │ (Go WS Bridge) │ │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └────────┬────────┘ │
└─────────┼────────────────┼────────────────┼──────────────────┼──────────┘
          │                │                │                  │
          └────────────────┴────────────────┴──────────────────┘
...
│  │  • NodeJS Proc  │  │  • BoltDB       │  │  • robfig/cron/v3       │  │
...
定时执行由管理后台的“定时任务”配置并注册到全局 `CRON` 调度器。插件文件注释不再解析定时字段，定时任务触发时按任务配置选择脚本和命令执行。
